...
public class WrapperPlayServerMapData extends PacketWrapper<WrapperPlayServerMapData> {
    ...
    public WrapperPlayServerMapData(int data, int scale, boolean locked, @Nullable Collection<Icon> icons, byte[] buffer, int x, int z, int column, int rows) {
    ...
    public void write() {
        ...
        boolean writeIcons = this.icons != null;
        if (this.clientVersion.isNewerThanOrEquals(ClientVersion.V_1_9)) {
            if (this.clientVersion.isOlderThan(ClientVersion.V_1_17)) {
        ...
        if (this.icons != null && (writeIcons || this.clientVersion.isOlderThan(ClientVersion.V_1_17))) {
